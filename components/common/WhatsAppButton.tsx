...
export default function WhatsAppButton() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-2 group">
      {/* Tooltip Popup */}
      {showTooltip && (
        <div className="relative bg-white text-slate-800 text-xs font-semibold px-3.5 py-2 rounded-xl shadow-xl border border-slate-200 flex items-center gap-2 animate-bounce">
          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
          <span>Need quick loan help? Chat on WhatsApp!</span>
          <button
            onClick={(e) => {
              e.stopPropagation();
          ...
          <div className="absolute -bottom-1.5 right-5 w-3 h-3 bg-white border-r border-b border-slate-200 transform rotate-45"></div>
        </div>
      )}
